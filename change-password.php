...
if (Login::isLoggedIn()) {
        if (isset($_POST['changepassword'])) {
                ...
                if (password_verify($oldpassword, DB::query('SELECT passwd FROM users WHERE id=:userid', array(':userid'=>$userid))[0]['passwd'])) {
                        if ($newpassword == $newpasswordrepeat) {
                                if (strlen($newpassword) >= 6 && strlen($newpassword) <= 60) {
                                        DB::query('UPDATE users SET passwd=:newpassword WHERE id=:userid', array(':newpassword'=>password_hash($newpassword, PASSWORD_BCRYPT), ':userid'=>$userid));
